...
class GraphAttnMultiHead(Module):
    ...
    def forward(self, inputs, adj_mat, requires_weight=False):
        support = torch.mm(inputs, self.weight) # 将输入inputs与权重矩阵self.weight进行矩阵乘法运算，得到支持矩阵support
        support = support.reshape(-1, self.num_heads, self.out_features).permute(dims=(1, 0, 2)) # 对support进行形状变换，将其变为三维张量，并进行维度置换，以便后续计算。
        f_1 = torch.matmul(support, self.weight_u).reshape(self.num_heads, 1, -1) # 将support与权重矩阵self.weight_u进行矩阵乘法运算，并进行形状变换，得到f_1。
        f_2 = torch.matmul(support, self.weight_v).reshape(self.num_heads, -1, 1) # 将support与权重矩阵self.weight_v进行矩阵乘法运算，并进行形状变换，得到f_2
        logits = f_1 + f_2 # 将f_1和f_2相加，得到logits，表示节点之间的关系得分。
        weight = self.leaky_relu(logits) # 将logits经过leaky_relu激活函数，得到权重矩阵weight。
        masked_weight = torch.mul(weight, adj_mat).to_sparse() # 将weight与邻接矩阵adj_mat相乘，并将结果转换为稀疏张量，得到masked_weight
        attn_weights = torch.sparse.softmax(masked_weight, dim=2).to_dense() # 对masked_weight进行稀疏矩阵的softmax操作，得到注意力权重attn_weights
        support = torch.matmul(attn_weights, support) # 将attn_weights与support进行矩阵乘法运算，得到支持矩阵support。
        support = support.permute(dims=(1, 0, 2)).reshape(-1, self.num_heads * self.out_features) # 对support进行维度置换和形状变换，将其变为二维张量。
        if self.bias is not None:
            '''
                如果存在偏置项（self.bias不为None），则将其加到support上。
                如果设置了残差连接（self.residual=True），则将输入inputs经过一个投影层self.project后的结果与support相加。
                如果requires_weight=True，则返回support和attn_weights；否则，返回support和None。
            '''
...
class PairNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.mode == 'PN':
            '''
            如果self.mode为'PN'，则进行以下操作：
            计算输入x在每列上的均值col_mean。
            将输入x减去列均值col_mean，得到经过均值中心化的结果。
            计算每行上的范数均值rownorm_mean，即对每行进行平方和求和，然后取平均，再加上一个微小值（1e-6），最后开平方根。
            将均值中心化后的结果乘以缩放因子self.scale，再除以范数均值rownorm_mean，得到最终结果。
        ...
        if self.mode == 'PN-SI':
            ...
            计算输入x在每列上的均值col_mean。
            将输入x减去列均值col_mean，得到经过均值中心化的结果。
            计算每行上的范数rownorm_individual，即对每行进行平方和求和，再加上一个微小值（1e-6），最后开平方根。
            将均值中心化后的结果乘以缩放因子self.scale，再除以每行的范数rownorm_individual，得到最终结果。
            '''
            x = x - col_mean
        ...
        if self.mode == 'PN-SCS':
            '''
            计算每行上的范数rownorm_individual，即对每行进行平方和求和，再加上一个微小值（1e-6），最后开平方根。
            将输入x乘以缩放因子self.scale，再除以每行的范数rownorm_individual，得到结果。
            将结果减去列均值col_mean。
...
class StockHeteGAT(nn.Module):
    def __init__(self, in_features=6, out_features=8, num_heads=8, hidden_dim=64, num_layers=1):
        ...
        for m in self.modules():  #对线性层的权重参数进行初始化，使用 Xavier 均匀分布初始化方法（nn.init.xavier_uniform_）。这种初始化方法是一种常用的权重初始化方法，旨在使得输入和输出的方差相等，并保持梯度的稳定性。gain=0.02是一个缩放因子，用于调整初始化的范围。 注意，这段代码只对线性层的权重参数进行初始化，不会对偏置项进行初始化。
