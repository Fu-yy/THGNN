...
class Args:
    def __init__(self, gpu=0, subtask="regression"):
        ...
        # save model settings
        self.save_path = PROJECT_PATH + os.sep + "data" + os.sep + "model_saved" + os.sep
...
def fun_train_predict(data_start, data_middle, data_end, pre_data):
    ...
    dataset = AllGraphDataSampler(base_dir=PROJECT_PATH + os.sep + "data" + os.sep + "data_train_predict/", data_start=data_start,
                               data_middle=data_middle, data_end=data_end)
    # val_dataset = AllGraphDataSampler(base_dir="/home/THGNN-main/data/data_train_predict/", mode="val", data_start=data_start,
    #                                   data_middle=data_middle, data_end=data_end)
    val_dataset = AllGraphDataSampler(base_dir=PROJECT_PATH + os.sep + "data" + os.sep + "data_train_predict/", mode="val", data_start=data_start,
                                      data_middle=data_middle, data_end=data_end)
    ...
    data_code = os.listdir(PROJECT_PATH + os.sep + "data" + os.sep + "daily_stock" )
    ...
    for i in tqdm(range(len(val_dataset))):
        df = pd.read_csv(PROJECT_PATH + os.sep + "data" + os.sep + "daily_stock" + os.sep + data_code_last[i], dtype=object)
    ...
    df_score.to_csv(PROJECT_PATH + os.sep + "data" + os.sep + "prediction" + os.sep + "pred.csv")
