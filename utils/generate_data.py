...
relation = os.listdir(PROJECT_PATH + os.sep + 'data' + os.sep + "relation" + os.sep)
...
def fun(relation_dt, start_dt_month, end_dt_month,df1):
    prev_date_num = 20
    adj_all = pd.read_csv(PROJECT_PATH + os.sep+"data"+ os.sep + 'relation'+os.sep + relation_dt+'.csv', index_col=0)
    # adj_all = pd.read_csv('/home/THGNN-main/data/relation/'+relation_dt+'.csv', index_col=0)
    adj_stock_set = list(adj_all.index)
    pos_g = nx.Graph(adj_all > 0.1) # 创建无向图
    ...
    for i in tqdm(range(len(dts))):
        ...
        for j in range(len(code)):
            ...
            if y.T.shape[1] != prev_date_num:
                ...
                mask.append(True)
                label = df3.loc[df3['dt'] == end_data]['label'].values # 最后一天的label的值
        ...
        with open(PROJECT_PATH + os.sep+ "data" + os.sep + 'data_train_predict'+os.sep+end_data+'.pkl', 'wb') as f:
            pickle.dump(result, f)
        df = pd.DataFrame(columns=['code', 'dt'], data=day_last_code)
        df.to_csv(PROJECT_PATH + os.sep+ "data" + os.sep + 'daily_stock'+os.sep +end_data+'.csv', header=True, index=False, encoding='utf_8_sig')
